...
const HeroCourse = () => {
  ...
  return (
      <div className='flex px-4 py-10 md:px-10 lg:px-20 xl:px-40 flex-1 justify-center bg-white'>
            ...
            <div className='flex flex-col-reverse gap-6 py-10 @[480px]:gap-8 @[864px]:flex-row items-center'>
                <div className='flex flex-col gap-6 @[480px]:min-w-[400px] @[480px]:gap-8 @[864px]:w-1/2 justify-center'>
                    <div className='flex flex-col gap-4 text-left'>
                        <div className='inline-flex w-fit items-center gap-2 rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-[#135bec] ring-1 ring-inset ring-blue-600/20'>
                            <span className='material-symbols-outlined text-[16px]'>verified</span> Ưu đãi 30% học phí hôm nay
                        </div>
                        <h1 className='text-[#111318] text-4xl font-black leading-tight tracking-[-0.033em] @[480px]:text-5xl lg:text-6xl'>
                            Vững tay lái <br/>
                        <span className='text-[#135bec]'>Trọn hành trình</span>
                        </h1>
                        <h2 className='text-[#616f89] text-base font-normal leading-relaxed max-w-[500px]'>
                    ...
                    <div className='flex flex-col sm:flex-row gap-4'>
                        ...
                        <button className='flex min-w-[84px] cursor-pointer items-center justify-center rounded-lg h-12 px-6 bg-[#f0f2f4] text-[#111318] text-base font-bold leading-normal tracking-[0.015em] hover:bg-gray-200 transition-colors'>
                            <span className='truncate'>Xem lịch khai giảng</span>
                        </button>
                    </div>
                </div>
                <div className='w-full @[864px]:w-1/2 relative'>
                    <div className='absolute -right-10 -top-10 h-64 w-64 rounded-full bg-blue-100/50 blur-3xl'></div>
                            ...
                            {slides.map((s) => (
                                <Link key={s.key} to={`/product/${s.productId}`} className='flex-shrink-0 block' style={{ width: `${100 / slides.length}%` }}>
                                    <div className='relative aspect-[4/3]'>
                                        ...
                                        <div className='absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-sm p-4 rounded-xl flex items-center justify-between border border-white/20 shadow-lg'>
                                            <p className='text-xs text-gray-500 font-medium'>Khóa học nổi bật</p>
                                            <p className='text-sm font-bold text-[#111318]'>{s.title}</p>
