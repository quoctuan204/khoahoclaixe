    ...
    <div className='py-16 bg-gray-50 min-h-screen'>
      <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8'>
        ...
        {loading ? (
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8'>
            {videos.map((video) => {
              const isUploaded = video.videoId && (video.videoId.includes('/uploads/') || video.videoId.startsWith('http'));
              return (
        ...
        {playingVideo && (
          <div className='fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4' onClick={() => setPlayingVideo(null)}>
            ...
            <div className='w-full max-w-5xl aspect-video bg-black rounded-lg overflow-hidden shadow-2xl' onClick={e => e.stopPropagation()}>
              {playingVideo.videoId && (playingVideo.videoId.includes('/uploads/') || playingVideo.videoId.startsWith('http')) ? (
                <video 
                  src={playingVideo.videoId.startsWith('http') ? playingVideo.videoId : `${API_BASE}${playingVideo.videoId}`} 
                  controls
                  autoPlay
